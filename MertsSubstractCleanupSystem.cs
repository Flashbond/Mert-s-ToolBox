...
{
    #region 1. SUBSTRACT MANAGER & DATA STRUCTURES

    /// <summary>
    /// Manages centralized requests for terrain and network cleanup operations.
    /// </summary>
    public static class SubstractManager
    {
        /// <summary>
        /// Defines the boundaries, transformations, and mathematical limits for a substraction operation.
        /// </summary>
        public struct SubstractRequest
        {
        ...
        public static SubstractRequest CurrentRequest;

        /// <summary>
        /// Queues a new substraction request with the specified spatial boundaries and rotation.
        /// </summary>
        public static void Request(float3 center, float rotation, float outerA, float outerB, float innerA, float innerB, float nValue)
        {
            ...
            CurrentRequest = new SubstractRequest
            {
    ...
    public partial class MertsSubstractCleanupSystem : GameSystemBase
    {
        ...
        /// <summary>
        /// Executes the substraction logic over active requests, evaluating and deleting entities that fall within the substraction core.
        /// </summary>
        protected override void OnUpdate()
        {
            if (SubstractManager.CurrentRequest.framesLeft <= 0)
                return;

            var req = SubstractManager.CurrentRequest;
            SubstractManager.CurrentRequest.framesLeft--;
            ...
            foreach (var entity in edgeEntities)
            {
                ...
                if (!IsNearSubstractArea(curve, req, AreaPaddingMain))
                    continue;
            ...
            foreach (var entity in curveOnlyEntities)
            {
                ...
                if (!IsNearSubstractArea(curve, req, AreaPaddingCurveOnly))
                    continue;
        ...
        /// </summary>
        private static float GetOuterValue(float3 worldPoint, in SubstractManager.SubstractRequest req)
        {
        ...
        /// </summary>
        private static bool IsInsideInner(float3 worldPoint, in SubstractManager.SubstractRequest req)
        {
            float3 local = math.transform(req.invMatrix, worldPoint);
        ...
        /// <summary>
        /// Performs a fast distance check to see if a curve is near the general substraction area.
        /// </summary>
        private static bool IsNearSubstractArea(Curve curve, in SubstractManager.SubstractRequest req, float padding)
        {
        ...
        /// </summary>
        private static bool LooksLikeOuterRing(Curve curve, in SubstractManager.SubstractRequest req)
        {
            float3 a = curve.m_Bezier.a;
            float3 m = MathUtils.Position(curve.m_Bezier, 0.5f);
            float3 d = curve.m_Bezier.d;
        ...
        /// </summary>
        private static bool ShouldDeleteMainCurve(Curve curve, in SubstractManager.SubstractRequest req)
        {
            float3 a = curve.m_Bezier.a;
            float3 m = MathUtils.Position(curve.m_Bezier, 0.5f);
            float3 d = curve.m_Bezier.d;
        ...
        /// </summary>
        private static bool ShouldDeleteCurveOnly(Curve curve, in SubstractManager.SubstractRequest req)
        {
            float3 a = curve.m_Bezier.a;
            float3 m = MathUtils.Position(curve.m_Bezier, 0.5f);
            float3 d = curve.m_Bezier.d;
